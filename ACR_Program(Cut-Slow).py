...
def callback():
    ...
    if(get_length(name)>seconds):
        ...
        try:
            ...
            video = video.resize(height=144)
            video = video.subclip(0, seconds)
            video.write_videofile(f'Logs/output{file_extension}')
        except:
            audio = AudioFileClip(name)
            audio = audio.subclip(0, seconds)
            audio.write_audiofile(f'Logs/output{file_extension}')
    ...
    textline.configure(state=DISABLED)
    try:
        func(f'Logs/output{file_extension}')
    except:
        func(name)
